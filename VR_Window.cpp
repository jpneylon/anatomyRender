...
VR_Window::VR_Window()
{
    ...
    vrender = new VRender;
    volume_origin = make_float3( 0, 0, 0 );
...
VR_Window::open_file()
{
    ...
    if (fp != NULL)
    {
        ...
        cloud->pcl.dimension.x = ( cloud->pcl.max.x - cloud->pcl.min.x );
        cloud->pcl.dimension.y = ( cloud->pcl.max.y - cloud->pcl.min.y );
        cloud->pcl.dimension.z = ( cloud->pcl.max.z - cloud->pcl.min.z );
...
VR_Window::read_socket_connection()
{
    ...
    int numItem = datasize / 4;
    for (int i = 0; i < numItem; i++)
    {
            dataPack[3*i+0] = buffer_ptr[4*i+0];
            dataPack[3*i+1] = buffer_ptr[4*i+1];
            dataPack[3*i+2] = buffer_ptr[4*i+2];

            dataPackColor[3*i+0] = (unsigned char)(255 * rgb_ptr[4*i+0] + 0.5);
            dataPackColor[3*i+1] = (unsigned char)(255 * rgb_ptr[4*i+1] + 0.5);
            dataPackColor[3*i+2] = (unsigned char)(255 * rgb_ptr[4*i+2] + 0.5);
    }
    ...
    {
        float3 position;
        position.x = dataPack[idx*3+0];
        position.y = dataPack[idx*3+1];
        position.z = dataPack[idx*3+2];

        uint3 rgb;
        rgb.z = dataPackColor[idx*3+0];
...
VR_Window::create_render_window()
{
    ...
    offset_label = new Gtk::Label("Difference: ");
    param_hbox2->pack_start(offset_label[0], false, false, 0);
    offset_adjust = Gtk::Adjustment::create( vrender->get_offset(), 0, 1.1, 0.01, 0.1, 0.1);
    ...
    scale_label = new Gtk::Label("Total: ");
    param_hbox2->pack_start(scale_label[0], false, false, 0);
    scale_adjust = Gtk::Adjustment::create( vrender->get_scale(), 0, 3.1, 0.01, 0.1, 0.1);
    ...
    //Glib::signal_timeout().connect( sigc::mem_fun(*this, &VR_Window::on_timer), 400 );
