                    ...
                    float offset, float scale, float dens )
{
    ...
    for( int i=0; i<maxSteps; i++)
    {
        ...
        int zero_check = white != 0;
        col.w = dens * __int2float_rn(zero_check);

        int cumulative_check = white > scale;
        col.w *= __int2float_rn(cumulative_check);

        float avg_color_diff = white / 3.f;
        float diff = fabs(red - avg_color_diff) + fabs(green - avg_color_diff) + fabs(blue - avg_color_diff);
        int diff_check = diff > offset;
        col.w *= __int2float_rn(diff_check);
               ...
               float scale )
{
    ...
    // clamp to [0.0, 1.0]
    float3 rgb;
    rgb.x = saturate( sum.x );
    rgb.y = saturate( sum.y );
    rgb.z = saturate( sum.z );

    // write output color
    d_output[3*(x + imageW * y) + 0] = (unsigned char) (255 * rgb.x);
    d_output[3*(x + imageW * y) + 1] = (unsigned char) (255 * rgb.y);
    d_output[3*(x + imageW * y) + 2] = (unsigned char) (255 * rgb.z);
}
                             ...
                             uchar  *blue )
{
    ...
    if (startIndex != 0xffffffff)
    {
        ...
        float point_count = 0; //__int2float_rn( endIndex - startIndex );
